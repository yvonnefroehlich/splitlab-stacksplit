...
% splitting paramters are parsed from the dialog created in SL_Results.m
...
if selected.Nulls(2)
    ...
    if license('checkout','MAP_Toolbox') %mapping toolbox
        rowNumbers=3;
    else
        rowNumbers=2;
    end
...
if rowNumbers==3
	...
	xlabel([{''},{config.stnname}],'FontWeight','bold')
    axis on
    %----------------------------------------------------------
    subplot(3,3,8)
    if length(k)>0
	...
	xlabel([{''},{sprintf('upper layer: %3.0f °, %.1f s', phi2, dt2)}],'FontWeight','bold')
	axis on
    % %----------------------------------------------------------
    subplot(3,3,9)
    if length(k)>0
    ...
    % xlabel('eigenvalue')
	xlabel([{''},{sprintf('lower layer: %3.0f °, %.1f s', phi1, dt1)}],'FontWeight','bold')
    axis on
end
...
%Label = xlabel(' ','FontWeight','Demi'); %halbfett; ??? woher SP fuer layer lower layer YF 25.12.2019
%----------------------------------------------------------
...
L=legend(e,'good non-null','fair non-null','poor','good null','fair null','Orientation','horizontal');
Pos = get(L,'Position');
%Pos(1:3)=[.2 .05 .6];
Pos(1:3)=[.22 .01 .6];
set(L,'Position',Pos),
...
%only if Non-Nulls are also displayed...
	...
	bazi=0:7.5:179;
    [phi0, dt0] = twolayermodel(bazi, phi1,dt1, phi2, dt2, period); %SplitLab function
    phi0 = [phi0; phi0];
    dt0  = [dt0; dt0];
    bazi = [bazi, bazi+180]';
    inc = ones(size(bazi))*10; %default 10deg inclination

    h = stereoplot(bazi, inc, phi0, dt0); %SplitLab function
    set(h,'Color','b')
    ...
    axis off
    hold off
    valuestring ={config.stnname; ...
				  sprintf('upper layer: %3.0f °, %.1f s', phi2, dt2); ...
				  sprintf('lower layer: %3.0f °, %.1f s', phi1, dt1)};
