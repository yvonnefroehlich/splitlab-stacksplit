...
% Update by RWP 10.23.2018 - changed filtering to use pole-zero form as in
% funclab/processRFmatlab
...
%%
    ...
    if thiseq.system=='ENV'
        %Amp = (filtfilt(b, a, E));
        Amp = filtfilt( sos, g, E);
        set(seis(1), 'Ydata',Amp)
        %Amp = (filtfilt(b, a, N));
        Amp = filtfilt( sos, g, N);
        set(seis(2), 'Ydata',Amp)
        %Amp = (filtfilt(b, a, Z));
        Amp = filtfilt( sos, g, Z);
        set(seis(3), 'Ydata',Amp)
    elseif thiseq.system=='LTQ'
        %Amp = (filtfilt(b, a, Q));
        Amp = filtfilt( sos, g, Q);
        set(seis(1), 'Ydata',Amp)
        %Amp = (filtfilt(b, a, T));
        Amp = filtfilt( sos, g, T);
        set(seis(2), 'Ydata',Amp)
        %Amp = (filtfilt(b, a, L));
        Amp = filtfilt( sos, g, L);
        set(seis(3), 'Ydata',Amp)
    end
